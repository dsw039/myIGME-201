...
namespace Recusion
{
    class Program
    {
        ...
        //Function that replaces the while loop in main. It continues sorting and calling itself until each value is sorted(length of Unsorted is = to 0 or less
        static void RecursiveSort(double[] aUnsorted, double[] aSorted, ref int nSortedLength, getArrayValueDelegate getArrayValue)
        {
            //This checks if there are values that are sorted or not. If there are not any, nothing happens. If there are, the sort begins/continues.
            if (aUnsorted.Length > 0)
            {
                //Everything here is mostly the same from the while loop. The main difference is the inclusion of the keyword ref and the code that makes the function call itself.
                double thisValue;
                ...
                ++nSortedLength;
                RecursiveSort(aUnsorted,aSorted,ref nSortedLength,getArrayValue);
            }
        }
        ...
        start:
            ...
            */
            //Call to a Recursive Function that replaces the old while loop
            RecursiveSort(aUnsorted, aSorted, ref nSortedLength, getArrayValue);
